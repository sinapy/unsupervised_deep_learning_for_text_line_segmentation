...
def get_backpaired_patches(img, thresh):
    ...
    if SHOW_RESULTS:
        ...
        cv2.rectangle(disp_img, (int(p1_pos[0]), int(p1_pos[1])),
                      (int(p1_pos[0] + patch_size), int(p1_pos[1] + patch_size)), (255, 0, 0), 5, lineType=cv2.LINE_AA)
        cv2.rectangle(disp_img, (int(p2_pos[0]), int(p2_pos[1])),
                      (int(p2_pos[0] + patch_size), int(p2_pos[1] + patch_size)), (0, 0, 255), 5, lineType=cv2.LINE_AA)
...
def get_different_patches(img, thresh):
    ...
    if SHOW_RESULTS:
        ...
        for cc_i in range(1, cc_number):
            x, y, w, h = cc_stats[cc_i][cv2.CC_STAT_LEFT], cc_stats[cc_i][cv2.CC_STAT_TOP], cc_stats[cc_i][
